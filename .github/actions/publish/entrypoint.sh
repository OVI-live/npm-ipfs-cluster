...
LATEST=`curl --silent https://dist.ipfs.tech/ipfs-cluster-follow/versions | tail -n 1 | cut -c 2-`
...
if [[ "$CURRENT" != "$LATEST" ]]; then
  ...
  if [[ $LATEST =~ "-" ]]; then
    NPM_DIST_TAG='next'
    echo "🧪 Found new ipfs-cluster-follow pre-release $LATEST@$NPM_DIST_TAG"
  else
    NPM_DIST_TAG='latest'
    echo "🎉 Found new ipfs-cluster-follow release $LATEST@$NPM_DIST_TAG"
  fi
  ...
  echo "📦 Published $LATEST to npm as ipfs-cluster-follow@$NPM_DIST_TAG"
